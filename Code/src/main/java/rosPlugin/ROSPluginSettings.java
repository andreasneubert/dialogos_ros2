...
public class ROSPluginSettings extends PluginSettings {
    ...
    public ROSPluginSettings() {
        System.out.println("TEST ROSPluginSettings:");
    }
    ...
    transient Multiset<String> publishableTopics = ConcurrentHashMultiset.create(); // publishableTopics are added from
                                                                                    // a propertyChangeListener in
                                                                                    // ROSNode
    transient Multiset<String> subscribedTopics = ConcurrentHashMultiset.create(); // publishableTopics are added from a
                                                                                   // propertyChangeListener in ROSNode
    ...
    public JComponent createEditor() {
        ...
        // FIXME: URI/IP should be disabled if FromENV is set. But then I won't be able
        // to use createPropertyPanel
        p.add(new PropertySet<Property<?>>(rosFromEnv, rosMasterURI, rosIP).createPropertyPanel(false),
                BorderLayout.NORTH);
        return p;
    }
    ...
    protected PluginRuntime createRuntime(Component component) {
        ...
        // "your_ros2_topic");
        // publisher.submit(new StringMessage().withData("your_initial_message"));

        PluginRuntime pr = new ROSPluginRuntime(
